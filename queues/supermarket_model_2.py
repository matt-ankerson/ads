...
# The scenario is: 10 checkouts, a customer selects a checkout based on the shortest queue
...
class SuperMarket(object):
    ...
    def add_customer(self):
        # assign a new customer to a queue based on shortest queue
        c = Customer(r.randint(1, 5))
        # selected quietest checkout
        quietest = self.checkouts[0]
        for checkout in self.checkouts:
            if len(checkout) < quietest:
                quietest = checkout
        quietest.enqueue(c)
...
if __name__ == "__main__":
    print("Shortest Queue Assignment: ")
    soup = SuperMarket()
    while not soup.done:
        soup.clock_a_minute()
